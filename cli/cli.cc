...
Iterator * CLI::indexnestedloopjoin(Iterator *input) {
  ...
  string tableNameStriped, attrNameStriped;
  getTableAttributeName(cond.rhsAttr, tableNameStriped, attrNameStriped);

//  IndexScan *right = new IndexScan(*rm, rightTableName, cond.rhsAttr);
  IndexScan *right = new IndexScan(*rm, rightTableName, attrNameStriped);
...
Iterator * CLI::createBaseScanner(const string token) {
  ...
  if (token.compare("IS") == 0) {
    ...
    string tableNameStriped, attrNameStriped;
    getTableAttributeName(cond.lhsAttr, tableNameStriped, attrNameStriped);
//    IndexScan *is = new IndexScan(*rm, tableName, cond.lhsAttr);
    IndexScan *is = new IndexScan(*rm, tableName, attrNameStriped);
...
//  void *buffer = malloc(tableName.size() + columnName.size()+8+4+1);
  ...
  memcpy((char *)buffer + offset, &is_variable, sizeof(int));
